...
pacman::p_load(tidyverse, jpeg, grid, rethinking, vroom)
```
...
foraging_fixations <- 
  read_csv('/System/Volumes/Data/CogSci/Semester 4/Eye tracking/Eye-Tracking-Workhop-CogSci-F2021/data/foraging_fixations.csv')
soceng_fixations <- 
  read_csv('/System/Volumes/Data/CogSci/Semester 4/Eye tracking/Eye-Tracking-Workhop-CogSci-F2021/data/soceng_fixations.csv')

## Load saccades data for both experiments
foraging_saccades <- 
  read_csv('/System/Volumes/Data/CogSci/Semester 4/Eye tracking/Eye-Tracking-Workhop-CogSci-F2021/data/foraging_saccades.csv')
soceng_saccades <- 
  read_csv('/System/Volumes/Data/CogSci/Semester 4/Eye tracking/Eye-Tracking-Workhop-CogSci-F2021/data/soceng_saccades.csv')
...
Saccades = Saccades %>%
  select(
    ...
    Blink= CURRENT_SAC_CONTAINS_BLINK, # Is this right?
...
```{r}
hist(Fixations_C$PositionX)
hist(Fixations_C$PositionY)
...
ggplot(x, aes(PositionX, PositionY)) +
  ...
  geom_point(aes(size = Duration), color = "firebrick", shape = 1) +
  theme_bw()
```
...
```{r}
Fixations_C <- Fixations_C %>% mutate(PositionY_flipped = max(PositionY) + 1 - PositionY)

Saccades_C <- Saccades_C %>% mutate(StartY_flipped = max(StartY) + 1 - StartY, 
...
```{r}
Fixations_C <- 
  Fixations_C %>% filter(Duration < sd(Duration)*3)

Saccades_C <- 
  Saccades_C %>% filter(PeakVelocity < sd(PeakVelocity)*3)
...
Fixations_C <- Fixations_C %>% 
  mutate(Order = str_extract(ParticipantID, '[:digit:]'))
...
logfiles_foraging <- logfiles_foraging %>% 
  mutate(Order = as.character(Order))

Fixations_C <- left_join(Fixations_C, logfiles_foraging, by = c("Trial", "Order"))


Saccades_C <- left_join(Saccades_C, logfiles_foraging, by = c("Trial", "Order"))
...
Fixations_C <- left_join(Fixations_C, logfiles_socialengagement, by = c("ParticipantID", "Trial"))

Saccades_C <- left_join(Saccades_C, logfiles_socialengagement, by = c("ParticipantID", "Trial"))
```
...
```{r}
# write_csv(Fixations_C, "data/Fixations_final.csv")
# write_csv(Saccades_C, "data/Saccades_final.csv")
