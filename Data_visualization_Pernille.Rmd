...
## Notes: 
Descriptive Plots - count/porportion and time metrics, ideal for summarizing them - that's frequency distributions and boxplots etc.

Scanpaths - sequences of fixations-saccades through time and space. Typically for one participant. Ordinal information (you can usually see which fixation came first), ideal for duration, length, duration, density direction etc. 
...
Fixations <- Fixations %>% filter(Fixations$Task == "Visual foraging")
...
```{r}
# Who do we have? 
unique(Fixations$ParticipantID)
unique(Fixations$Trial)
```

And here is one from the Count condition:

```{r}
make_scanpath("sophie1",6)
...
make_heatmap_cumulative <- function(conditions, trial){
  
  ## Select only the participant/trial of interest
  x <- Fixations[Fixations$Trial == trial & Fixations$SearchOrCount == conditions,]
  condition <-x$SearchOrCount
...
make_heatmap_cumulative("Search", 8)
make_heatmap_cumulative("Count", 8)
...
# Trying to do it with the dolphin
## Define an AOI for the star
AOI = c(520, 880, 60, 320)

x <-
  Fixations %>% filter(Duration >= 300 &
                         PositionX >= AOI[1] &
                         PositionX <= AOI[2] & PositionY_flipped >= AOI[3] & PositionY_flipped <= AOI[4])
...
ggplot(x, aes(PositionX, PositionY_flipped, color = FixationNo)) + 
  annotation_custom(img, xmin = 0, xmax = 1680, ymin = 0, ymax = 1050) +
  # this line draws the rectangle for the AOI
  geom_rect(xmin=AOI[1], xmax=AOI[2], ymin=AOI[3], ymax=AOI[4], fill = NA, size = 1, color = 'red') +
  stat_density2d(geom = "raster", aes(fill = ..density.., alpha = sqrt(sqrt(..density..))), contour = FALSE, n = 1000) + 
...
```{r}
# Changing the function to be with both conditions
make_heatmap_cumulative_both <- function(trial){
  
  ## Select only the participant/trial of interest
  x <- Fixations[Fixations$Trial == trial,]
  ...
  p <- ggplot(x, aes(PositionX, PositionY_flipped, color = FixationNo)) + 
    ...
    scale_alpha(range = c(0.1, 0.6)) + xlim(0,1680) + ylim(0,1050) + theme(legend.position = "none") +
    theme_light()+
      geom_rect(xmin=AOI_dolphin[1], xmax=AOI_dolphin[2], ymin=AOI_dolphin[3], ymax=AOI_dolphin[4], fill = NA, size = 1, color = 'red') +
  stat_density2d(geom = "raster", aes(fill = ..density.., alpha = sqrt(sqrt(..density..))), contour = FALSE, n = 1000) + 
  scale_fill_gradientn(colours = heat_colors(10), trans="sqrt") +
  scale_alpha(range = c(0.1, 0.6)) +
  xlim(0,1680) + ylim(0,1050) +
...
make_heatmap_cumulative_both(7)
...
x <- Fixations %>%
  ...
  mutate(
    InAOI1 = ifelse(
      PositionX >= AOI1[1] &
        PositionX <= AOI1[2] &
        PositionY_flipped >= AOI1[3] & PositionY_flipped <= AOI1[4],
      TRUE,
      FALSE
    ),
    InAOI2 = ifelse(
      PositionX >= AOI2[1] &
        PositionX <= AOI2[2] &
        PositionY_flipped >= AOI2[3] & PositionY_flipped <= AOI2[4],
      TRUE,
      FALSE
    )
  )
...
# Only first fixation in AOI1
(TTFF_AOI1 <- x %>% 
    filter(InAOI1 & StartTime != 4) %>%
  group_by(ParticipantID) %>%
  slice(1)) %>% 
  arrange()

TTFF_AOI1 %>% 
  ggplot(aes( x = reorder(ParticipantID, -StartTime), y = StartTime, fill = StartTime))+
  geom_bar(stat = 'summary', fun.y = mean, width = 0.5)+
  facet_wrap(.~SearchOrCount)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  ggtitle("TTFF for both conditions")
